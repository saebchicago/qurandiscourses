...
- It is **not** AI-generated commentary passed off as a human reading. If
  you used an AI tool to help draft or check your submission, say so in
  the PR description — that's fine, but it must be disclosed, and the
  underlying claims still need to trace to real data or a real citation.
...
2. **No unlabeled AI conclusions.** Any computational output (candidate
   boundaries, theme suggestions) is labeled "candidate signal,
   unverified" in the data layer, not just implied by placement on the
   page.
