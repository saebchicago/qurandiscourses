...
| build-quran-text.mjs | alquran.cloud `quran-uthmani` + Tanzil Uthmani download (network) | data/quran-text/{1-114}.json, data/quran-text/index.json | the bundled Arabic text for read.html. Writes nothing unless all 6,236 verses match Tanzil's own file (the only tolerated difference: the basmala, Tanzil's 1:1 text, before verse 1 of surahs other than 1 and 9), and copies Tanzil's notice verbatim into the index. Needs the network, so `.github/workflows/quran-text.yml` runs it: on a working branch it commits the result; on main it runs monthly with `--check` and fails on upstream drift, never committing |
...
| build-root-refs-index.mjs | roots-summary, the pages that load `assets/refs.js` | assets/root-refs/{page}.js | refs.js root-mention detection, one slim whitelist PER PAGE holding only the tokens that page's own markup could match (ambiguous ASCII folds deliberately absent). Adding a root mention to one of those pages' prose means re-running this — `check-generated-freshness` catches a stale table. A page with nothing to wrap is listed in the generator's `OPTED_OUT` and loads neither script; the generator fails if such a page later gains a mention |
...
| check-data-nums.mjs | THREE families. (1) every `data-num="dot.path"` binding across every page: the path must resolve to a number in `data/numbers.json`, and the element's static fallback text must match that number under `initDataNums()`'s own formatting — catches a stale prose figure or a typo'd path, both of which `initDataNums()` fails on silently in the browser (it only overwrites when the path resolves to a number) (2) coverage.html's report-backed fallbacks, whose ids are read from the page itself. (3) THE INVERSE: a comma-formatted number >= 1,000 in page prose that equals a numbers.json value must BE bound, or be named in the checker's printed skip list with a reason — the convention was applied unevenly, and the first two families only ever saw figures a page had already opted into. changelog.html is exempt as a whole: its entries are a historical record, frozen at the date of the entry |
...
| check-egress.mjs | whether THIS machine can reach the three third-party hosts the site and its network checkers depend on — api.alquran.cloud (verse text), cdn.islamic.network (recitation audio), everyayah.com (the archive /sources cites). Probes a path with a recognisable payload on each, because a proxy denial and a service outage answer a root request identically. Advisory by default (`--strict` to exit 1). Run it FIRST when a "needs real outbound network" checker fails: it separates "this sandbox has no egress" from "the third party is down", which otherwise look the same. In an agent session or any restricted environment, these three hosts are what an egress allowlist has to name |
...
| check-static-server.mjs | local preview smoke test: starts `scripts/lib/static-server.mjs` (the dev server behind `scripts/serve.mjs`, used by the Replit preview) and asserts key public paths answer 200. Local tooling only; it does not exercise Netlify headers or redirects |
